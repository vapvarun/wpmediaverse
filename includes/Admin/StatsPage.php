...
class StatsPage {
	...
	public function render_page(): void {
		...
		// Build tabs — Pro can inject "Video Analytics" via this filter.
		$tabs = array(
			'overview' => array(
				'label' => __( 'Overview', 'wpmediaverse' ),
			),
		);

		/**
		 * Filter stats page tabs.
		 *
		 * Pro plugin uses this to inject the "Video Analytics" tab.
		 * Each tab entry: 'slug' => array( 'label' => string, 'callback' => callable ).
		 *
		 * @param array $tabs Tab definitions.
		 */
		$tabs = apply_filters( 'mvs_stats_tabs', $tabs );
		...
		<div class="wrap wpmediaverse-admin">
			...
			<?php // Show tabs only when more than 1 tab exists. ?>
			<?php if ( count( $tabs ) > 1 ) : ?>
			...
			} elseif ( isset( $tabs[ $active_tab ]['callback'] ) && is_callable( $tabs[ $active_tab ]['callback'] ) ) {
