...
class Plugin {
	...
	// lives in \WPMediaVerse\Core\Plugin::container()->get( 'media_repository' )->get($id, 'file_url'/'thumb_*')
	...
	public static function enqueue_frontend_assets(): void {
		...
		// mvs-rest.js calls window.wp.i18n.__() directly (album-name errors,
		// network/create-failure toasts). As a classic script it CAN use
		// wp_set_script_translations — unlike our Interactivity modules, which
		// inject PHP-translated strings via wp_interactivity_state() instead.
		// The languages/ path is passed so the plugin's own bundled .json
		// (generated by `wp i18n make-json` at build) resolves, not just
		// wp-content/languages/plugins. Basecamp 10073528834.
		wp_set_script_translations( 'mvs-rest', 'wpmediaverse', MVS_PLUGIN_DIR . 'languages' );
		...
		// load-more.js surfaces one translatable string (the retry error).
		// Classic script — wire wp_set_script_translations like mvs-rest.
		wp_set_script_translations( 'mvs-load-more', 'wpmediaverse', MVS_PLUGIN_DIR . 'languages' );
		wp_register_style(
			'mvs-load-more',
	...
	public static function enqueue_messaging_assets(): void {
		...
		// an @wordpress/i18n import — it imports only @wordpress/interactivity —
		// so wp_set_script_module_translations() had nothing to feed and was
		// removed. Same PHP-injected-i18n approach as the Interactivity stores.
	...
	public static function print_messaging_config(): void {
		...
		$config = array(
			...
			// PHP-translated strings for messaging.js, keyed by their English
