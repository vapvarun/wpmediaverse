...
class BuddyPressIntegration {
	...
	/**
	 * Add a Media tab on the user profile.
	 */
	public function add_profile_tab(): void {
		...
		bp_core_new_nav_item(
			array(
				...
				'slug'                => 'media',
				'parent_url'          => bp_displayed_user_domain() . 'media/',
	...
	public function profile_media_content(): void {
		...
		if ( empty( $media ) ) {
			echo '<p>' . esc_html__( 'No media items found.', 'wpmediaverse' ) . '</p>';
			return;
		}

		echo '<div class="mvs-media-grid mvs-cols-3">';
		foreach ( $media as $item ) {
			$file_url  = get_post_meta( $item->ID, '_mvs_file_url', true );
			$file_type = get_post_meta( $item->ID, '_mvs_file_type', true );
			$is_image  = $file_url && strpos( $file_type, 'image/' ) === 0;

			echo '<div class="mvs-grid-item">';
			if ( $is_image ) {
				echo '<a href="' . esc_url( get_permalink( $item->ID ) ) . '">';
				echo '<img src="' . esc_url( $file_url ) . '" alt="' . esc_attr( $item->post_title ) . '" loading="lazy" />';
				echo '</a>';
			} else {
				echo '<div class="mvs-grid-item-placeholder"><span class="dashicons dashicons-media-default"></span></div>';
			}
			echo '<div class="mvs-grid-item-overlay"><span>' . esc_html( $item->post_title ) . '</span></div>';
			echo '</div>';
		}
		echo '</div>';
	}
