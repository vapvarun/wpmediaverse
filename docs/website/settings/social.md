...
Access these settings at **WPMediaVerse > Settings > Social**.
...
|--------|-----|---------|-------------|
| Who Can Send DMs | `mvs_dm_access` | `everyone` | Controls who is allowed to send direct messages to other users on the site. Options: `everyone`, `followers` (people who follow the recipient; others go to Requests), `mutual` (only mutual followers), `nobody` (DMs disabled). |
...
| Chat Panel Visibility | `mvs_chat_panel_visibility` | `everywhere` | Where the floating chat panel renders. Options: `everywhere`, `mvs_pages` (WPMediaVerse pages only - Explore, Dashboard, Albums, Member Profiles), `bp_pages` (BuddyPress member + group pages only), `disabled`. For code-level overrides, hook into the `mvs_should_render_chat_panel` filter - return `false` to suppress the panel on a specific request. |
