...
| `mvs_rest_can_access` | filter | `REST/CommunityPrivacyGate.php` | When the gate is armed, decides whether the request passes (default `is_user_logged_in()`); blocked callers get `401 mvs_community_private`. Since 2.2.0. |
| `mvs_rest_gated_route_prefixes` | filter | `REST/CommunityPrivacyGate.php` | Route prefixes the gate covers (default `array('/mvs/v1/')`); Pro appends `/mvs-pro/v1/` from its Plugin.php boot. Since 2.2.0. |
