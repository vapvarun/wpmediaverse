...
* New     - Frontend `<picture>` rendering. Explore grid, BP activity feed, dashboard cards, single-media view, AND the Interactivity-API lightbox automatically serve WebP via `<picture><source type="image/webp">` when a sibling exists. Older browsers fall back to the original. Lightbox added in 1.3.0.
* New     - AVIF variant generation alongside WebP. Opt-in setting `Generate AVIF copies` on the Storage tab (default off because AVIF encoding is 10x slower than WebP). Sites that turn it on get an AVIF sibling at the original + every thumbnail size. AVIF is typically 25 to 50 percent smaller than WebP for the same visual quality. AVIF-capable browsers (Chrome, Firefox, Safari 16.4+, Edge) pick AVIF; older browsers fall through to WebP, then the original. Stored as `original_avif` and `thumb_<size>_avif` meta keys.
* New     - Gated `/serve` route negotiates WebP and AVIF via the `Accept` header. Private media protected by signed URLs now ALSO serve modern formats when the client advertises support. The same signed URL works for all three formats; `Vary: Accept` keeps shared caches honest. Filter: `mvs_avif_quality` (default 50).
...
* Fix     - Video uploads without an embedded cover atom (most MP4s from desktop editors, some screen recordings) now generate per-size posters + WebP siblings correctly instead of falling back to a single unsized JPEG. Root cause: PHP-FPM inherits a minimal PATH from the launching service (Local, MAMP, managed hosts) that excludes Homebrew + non-standard ffmpeg locations, so `proc_open(['ffmpeg', ...])` exited 127 silently and the upload fell through to the client-uploaded canvas thumbnail. Fix: new `UploadService::resolve_ffmpeg_binary()` probes `/opt/homebrew/bin/ffmpeg`, `/usr/local/bin/ffmpeg`, `/usr/bin/ffmpeg`, `/opt/ffmpeg/bin/ffmpeg` before falling back to bare `ffmpeg`. Failure path now logs a clear warning. The `mvs_ffmpeg_binary` filter still wins for explicit overrides.
* New     - Default video poster for cover-less uploads. When all four poster sources fail (no embedded cover atom, no ffmpeg, no Pro transcode, no client-side canvas frame), the grid and lightbox now render a plugin-bundled SVG placeholder instead of a black `<video>` element. The SVG ships in `assets/images/default-video-poster.svg` (single 2.7 KB asset, browser-cached site-wide), is rendered at HTML template time, and is NEVER written to `mvs_media_meta` or pushed to cloud storage. Filter: `mvs_default_video_poster_url` for theme-matching overrides. Mirrors the existing audio waveform SVG fallback pattern.
* Improve - Client-side canvas thumbnails (the JS `<video> + canvas + toDataURL` frame the browser sends with video uploads) now flow through `UploadService::generate_thumbnails()` for per-size resize + WebP/AVIF siblings instead of being dumped as a single unsized `wpmediaverse/thumbs/video-thumb-{id}.jpg`. Pre-existing legacy files in that directory continue to work; admins can run `wp mvs generate-video-thumbnails --force` to upgrade them to the new flow when convenient. The retired path was a Pre-1.2.2 fallback that bypassed image optimization entirely.
* Dev     - New `mvs_optimize_image`, `mvs_optimize_jpeg_quality`, `mvs_webp_quality`, `mvs_avif_quality`, `mvs_ffmpeg_binary` filters. Documented in `docs/architecture/specs/2026-05-17-image-optimization-pipeline.md`.
