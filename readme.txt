...
* Fix     - Private media is no longer exposed on other users' profile tabs. Profile views now apply viewer-aware privacy filtering by default, so private uploads stay hidden from anyone who is not the owner, a moderator, or an explicitly granted viewer.
* Fix     - Pro watermark now applies to all non-public uploads when watermarking is enabled. Previously the watermark only fired when the media had an explicit access rule, which left vanilla private, friends-only, and group uploads watermark-free. A new mvs_watermark_applies filter lets sites restore the pre-1.5.0 access-rules-only behavior if needed.
