...
### C.member.documents-drive (Pro; combo only)
**What to verify:** as a MEMBER, never as admin. `/my-media/` offers a Documents section and `/my-media/documents/` opens the drive. A folder can be created (a duplicate name is refused by name, not with a generic failure), a document uploads into it and lands *in* that folder, and the row offers rename, move, privacy, download and trash. Trash then restore returns it to the same folder. In-drive search matches on the document's TEXT, not only its title. Multi-select bulk move reports refusals with the real reason. Breadcrumbs and the folder header carry the folder's own actions, and filter / sort / view choices survive opening a folder, paging and following a breadcrumb.
**Why it matters:** this is 2.4.0's headline feature and it had no row in this runbook until 2026-08-11 — the release gate could go green having walked no document surface at all.

### C.member.documents-privacy-and-sharing (Pro; combo only)
**What to verify:** a new upload lands on the configured default privacy (`private` unless the owner changed it). The per-row privacy dropdown offers exactly Only me / Members / Anyone — if it offers `unlisted`, the vocabulary has regressed, because nothing writes that value. Sharing a document with a specific member or a role grants access, the grantee sees it in **Shared with me**, and revoking removes it. A grantee with `edit` cannot hand the document on. A document shared by grant must OPEN, not merely appear in the list — being listed and then 403'd on open was a real 2.4.0 bug.
**Why it matters:** the listing and the page disagreeing about who may read is the failure this pair exists to catch.

### C.member.documents-role-gate (Pro; combo only)
**What to verify:** with the member's role ticked under Settings → Documents → *Who can use documents*, the Documents section is present and `mvs-pro/v1/documents`, `/folders` and `/me/shared` all answer 200 for them. Untick that role and save: the section disappears and all three routes answer **403 `mvs_documents_unavailable`** — not 401, because the member IS signed in. Re-tick: the section returns and every folder and document is exactly where it was. Nothing is deleted at any point. Separately, a logged-out visitor must still be able to open an already-public document and the Explore Documents listing — the capability governs whether a member has a library, never whether anyone may READ a document they could otherwise see.
**Why it matters:** gating a read would make a public document visible to a visitor and invisible to a signed-in member.

### C.member.documents-single-and-public (Pro; combo only)
**What to verify:** `/media/{slug}/` for a document renders by preview tier — PDF inline, text/markdown/csv as server-rendered HTML, word/excel client-side, the rest as a card plus Download. A missing file says "the file for this document is missing", not an empty box. The public `/explore-document/` listing shows rows (never grid tiles) with type chip, size and author, and pages correctly. A refused document answers **404**, never 403 — a filename can carry a client's name.

### C.admin.documents-screen (combo only)
**What to verify:** wp-admin → WPMediaVerse → Documents lists documents with search, type and privacy filters and working sort. Row actions are Edit / View on site / Download / Trash / Delete permanently, and the Title opens the editor rather than leaving for the front end. The single view at `?page=mvs-documents&view=single&media_id=N` edits title, slug, description, tags and privacy; it refuses a photo id and an empty title; and **the slug is never regenerated from the title** — a member fixing a typo must not break every link they have shared.

### C.admin.documents-settings (combo only)
**What to verify:** Settings → Documents renders seven controls — Enable Documents, Who can use documents, Maximum document size, Allowed file types, New documents start as, Anonymous share links, Search inside documents — and each shows the value it will actually use BEFORE any save (an unsaved default-ON toggle must not render as off). Each setting then changes both what the server enforces and what `GET /mvs/v1/app/config` → `documents` advertises, in the same direction: size (`max_size`, `0` = follow the server), types (`allowed_types` AND `allowed_mimes`, and the drive's type filter stops offering the refused type), default privacy (`default_privacy`), anonymous links (`anonymous_links`, and switching it off closes links already issued), extraction (`search`). Filters still override the settings — the resolution order is option first, filter last.
**Why it matters:** three values in `/app/config` were hardcoded or read from the wrong source, so the app advertised behaviour the server refused. Config that disagrees with enforcement is the defect this row exists to catch.
...
| D.documents-role-gate-hides-not-deletes | `use_mvs_documents` was introduced on an already-shipped feature. A default-denied capability, or a gate that deleted rather than hid, would empty every existing member's drive on update. | Every role on the site — including roles registered by BuddyPress / WooCommerce — has `use_mvs_documents` on an untouched install. Untick a role: its members get 403 `mvs_documents_unavailable` from `/documents`, `/folders`, `/me/shared` and lose the dashboard section. Count the member's documents before and after: **identical**. Re-tick: drive intact. Then run `MediaCapabilities::add_caps()` (what a version bump runs) and confirm a revoked role stays revoked — the override replay is what stops an update overturning the owner. |
| D.documents-config-matches-enforcement | `/app/config` reported `anonymous_links: true` while the server refused every mint, `default_privacy` was hardcoded, and `allowed_mimes` was published unfiltered beside a filtered `allowed_types`. A client draws its UI from this block and cannot discover the truth any other way. | For each of size / types / default privacy / anonymous links / extraction: change the setting, then confirm `GET /mvs/v1/app/config` → `documents` moves in the same direction AND the server enforces the same thing. `enabled` is PER USER — a member whose role has documents off must see `enabled: false`, not a tab that 403s. |
| D.documents-privacy-vocabulary | The vocabulary had four definitions that disagreed: the constant declared `private\|members\|unlisted`, while `DriveActions` (twice) and the drive's row dropdown wrote `private\|members\|public`. Nothing ever wrote `unlisted`; `public` was written and undeclared. | `DocumentSettings::PRIVACY_VALUES` is `private, members, public`. The drive's row dropdown offers exactly those three with the same labels. `POST /documents/{id}` with `unlisted` or a media-only level (`restricted`) answers 400 `mvs_document_privacy_invalid`. No surface offers `unlisted`. |
| D.document-never-in-media-surface | A quarantined PDF rendered as a broken tile in Explore. Grids draw pictures; a document has none. | Explore, the media grid, album items, activity and the admin All Media list all carry POSITIVE type predicates. `/mvs/v1/media?media_type=document` answers **400 `mvs_document_route`**, with `mvs_media_feed_allows_documents` as the documented escape hatch. Release-blocker journey: `audit/journeys/security/07-document-never-in-media-surface.md`. |
| D.media-trash-silent | `MediaRepository::trash()` fired no action, so an integration mirroring media had nothing to listen to — a member trashed a video and the feed went on advertising it, linking to a 404. Permanent delete always worked; trash never did. | Probe `mvs_media_trashed` / `mvs_media_restored` via mu-plugin. Trash from ALL FOUR paths — member REST, admin row action, admin bulk, and the two Documents admin paths — and assert an event per path with a NON-EMPTY `$permalink`. The admin paths are the point: they wrote the status column directly and were silent while the member path worked. Documents must additionally fire `mvs_document_trashed` from the admin screens. (Basecamp 10252324048.) |
| D.drive-access-media-vs-doc | One filter (`mvs_document_drive_access`) answered both "may this member add a FILE here" and "may they add a PHOTO here", so a Space with its files tab off had no media drive either, and space-scoped media privacy was unreachable. | With a bridge answering `none` for documents and `write` for `mvs_media_drive_access`: an upload lands on `drive_type='space'`, AND a second member can `can_view()` that media once its privacy is `space`. **Both halves or it fails** — placement succeeding while the read is refused means media is stored somewhere its own members cannot open. (Basecamp 10252314484.) |
| D.anon-doc-read-void | `/documents` refused every anonymous request whatever the ladder granted, so `read` was a level a bridge could hand out and the route would silently void. | Bridge answers `read` for a Space drive. With `mvs_pro_documents_anon_links=0`: signed-out GET `/mvs-pro/v1/documents?drive=space:N` → **401**. With it `=1` → **200**. With the ladder switched to `none` and the option still on → **not 200**. Signed-out POST `/documents/upload` → **401** always. `/me/shared` and `/drives` → never 200 signed-out. Reset the option to 0 afterwards. (Basecamp 10252332749.) |
| D.avatar-flag-contradiction | `has_custom_avatar` reported `false` for a member whose real photograph the `avatar` field beside it was serving, because the flag read only our own store. Any upload-a-photo nudge then asked for a picture they already had. | With a second avatar provider active, for a member whose avatar THAT plugin stores: `has_custom_avatar` is `true`. For a member with no avatar anywhere it is still `false` — a seam that returns true for everyone is the same bug pointing the other way. (Basecamp 10252323883.) |
