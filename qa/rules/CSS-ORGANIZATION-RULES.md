 ...
 * RTL:         <not generated - see below; note any hand-authored [dir="rtl"] block>
 *
...
### A conditional override must be declared AFTER what it overrides

Custom properties carry no specificity, so the cascade falls back to document
order: the LAST declaration wins. That makes `@supports` overrides
order-sensitive in a way that fails silently.

```css
:root { --token: <legacy form>; }            /* base */

@supports (color: contrast-color(red)) {
    :root { --token: contrast-color(...); }  /* WINS - declared later */
}
```

Declare the base value *after* the `@supports` block and the override never
applies. Nothing errors, no gate complains, and the token quietly keeps the
value the override existed to replace.

This is not hypothetical. `--mvs-on-accent` was added next to the other accent
tokens, several hundred lines below the `@supports` block that was meant to
correct it. The base form measured as WHITE for that indigo - precisely the
4.47:1 contrast failure the token was introduced to fix - while the override sat
inert higher up the file. It was confirmed by injecting a late declaration in
the browser: the computed value flipped immediately, proving order was the only
cause. The fix was to move the base declaration up beside
`--mvs-on-primary-auto`, which had always worked for exactly this reason.

**Check when adding a token that an `@supports` block overrides:** its base
declaration must appear earlier in the file than that block. If you are adding
the base near related tokens further down, move it up instead of trusting the
override.

### RTL: do not reach for rtlcss

`grunt-rtlcss` was removed from the build in 2.3.0 and must not come back.
`Gruntfile.js:347-355` records why: it generated `assets/css/*-rtl.css` that
nothing ever enqueued (there is no `wp_style_add_data( $handle, 'rtl', 'replace' )`
in either plugin - verified 0 occurrences), and because themes already set
`dir="rtl"`, serving a flipped sheet DOUBLE-flips the layout. That was measured
on a real Arabic install: registering the files moved the Explore heading and
tag chips back to left-aligned on an RTL page.

What this file's header field used to say - "auto-generated by grunt-rtlcss" -
described a pipeline that no longer exists, and would send the next author
after the wrong tool. Nothing in `assets/css/` carried that header anyway.

RTL correctness comes from two places instead:

1. The browser mirrors flex, grid and `text-align: start/end` from `dir="rtl"`
   on its own. Most of our layout needs nothing.
2. Physical properties never mirror. `left`, `right`, `margin-left/right`,
   `padding-left/right` describe a side of the SCREEN, not a side relative to
   reading direction. Prefer `inset-inline-start/end`, `margin-inline-*` and
   `padding-inline-*` in new rules - they flip for free.

`border-left`/`border-right` are deliberately out of that scope; where they
matter they keep their existing `[dir="rtl"]` overrides.
