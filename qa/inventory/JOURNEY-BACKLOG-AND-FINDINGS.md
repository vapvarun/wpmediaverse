...
| F2 | High | FIXED (core) | Block-user was REST-only with no frontend affordance. Added a Block/Unblock toggle to the profile actions (next to Follow/Message), wired to `POST/DELETE /users/{id}/block` with feedback and state; browser-verified (block -> server `/me/blocked` reflects it -> unblock reverts). Remaining enhancement: a dedicated blocked-list management view (unblock is currently reachable from each profile). | `templates/partials/profile-actions.php`, `assets/js/frontend/profile-actions.js` |
