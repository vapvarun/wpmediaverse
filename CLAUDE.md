...
|------|--------|---------|
| 2026-08-19 | 2.4.0 | **The index v29 added was never used, and one guard made that permanent (paired with Pro 2.4.0; NOT released).** The phase-11 plan deferred a decision — keep `doc_listing` or drop it once `drive_listing` exists. Measuring it on a 30,000-document fixture answered it (**keep**: a folder listing carries no drive predicate, so it is `doc_listing` verbatim, and `drive_listing` has `drive_type`/`drive_id` at positions 2 and 3 where a folder listing cannot use them) and found two defects underneath. **(1) `drive_listing` was not the index the migration claims to build.** On a real database it was a byte-identical duplicate of `doc_listing`. `add_index_if_missing()` tested the key NAME and never its columns — and **dropping a column does not drop the composite index it belongs to; MySQL rewrites it down to the surviving columns**. The 2026-08-15 upgrade rehearsal did exactly that on purpose; a restored dump or a host tool does it by accident. The columns come back, the guard sees the name, reports success, and the six-column index is never rebuilt — the site is left permanently missing an index its own comments promise it has, with nothing to indicate it. The guard now compares the column list and rebuilds on mismatch (two tests: one degrades an index and asserts the rebuild, one asserts a correct index is left alone, because rebuilding on every run is an expensive no-op on a large table). **(2) Even built correctly it served nothing.** The root query ORed the drive predicate with a legacy `post_author` branch for rows the bounded backfill had not stamped, and an OR cannot satisfy positions 2 and 3 of a composite index — the optimiser SAW `drive_listing` in `possible_keys` and refused it. The legacy branch is now emitted only while the backfill runs; **absent reads as "still running"**, because reading it as "done" would scope every listing by a column nothing has written and make every drive look empty. Measured, one page at OFFSET 1000: **8,032 rows examined at 1.38% filtered -> 234 at 100%**, `doc_listing` -> `drive_listing`. Closes the deep-OFFSET soft spot `drive_documents()` has carried a comment about since the feature was written; the remaining one is `any_folder` (the Recent view), which drops `folder_id` and so reads only three columns. **Also:** the Pro licence gate gained a cert oracle, which needed two additions to `Cert\CertRunner` — `kind: license` (a licence is not a checkbox; writing '1' reads as "no licence" exactly like '0', so an option-kind oracle would flip nothing and report the gate dead) and **`as_role`** (the runner signs in as user 1, and a gate that deliberately exempts administrators cannot be proved by an admin dispatch — it succeeds, and the oracle calls the gate dead while it works as designed). The oracle's first target created a real folder on every run, including one orphaned on a deleted throwaway user's drive; it now restores an id that cannot exist and creates nothing. **Docs:** the document library shipped with no customer page at all — `docs/website/pro-features/documents.md` added. Free 430 tests, cert 69/0/0, CI green. Plan: `plan/document-library.md` §27. |
