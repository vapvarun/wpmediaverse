...
| 2026-03-10 | BP Fixes (10) | Activity thumbnails in stream, media posting button in "What's New" (REST upload + event delegation for Nouveau), register activity types for filter dropdown, BP Nouveau notification filter types (bp_nouveau_notifications_register_filter), album cover fallback images, media count in tab label (bp_template_redirect + edit_nav), HTTPS mixed content + attachment fallback on profile/group grids, reaction activity dedup, bp_is_active() safety guards. New: assets/js/bp-activity-media.js, assets/css/frontend.css section 24. |
| 2026-03-10 | BP-11 Lightbox | Instagram-style interactive lightbox for activity media — reactions (toggle), comments (post/list), favorites (toggle), share (clipboard/native), stats, "View Full Page" link. Slug-based media lookup via REST `?slug=` param. CSS section 25. Selector fix: BP strips `<div>` from activity content, so JS matches `.activity-content a[href*="/media/"]` instead of `.mvs-activity-media a`. |
