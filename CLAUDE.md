...
| Date | Commit | Summary |
|------|--------|---------|
| 2026-05-07 | 1.2.1 (cloud + i18n) | **Cloud-storage tooling end-to-end + comprehensive i18n sweep + privacy gate (single-day push).** Cloud: new `Services\CloudOps` (per-row migrate / cleanup-local / count helpers — shared by CLI + Pro admin UI), `Services\StorageDriverInterface::download()` contract method, three new CLI subcommands `wp mvs migrate-storage` / `wp mvs cloud-thumbs-backfill` / `wp mvs cleanup-local`, Stage 3.5 of migrate refreshes `mvs_media_index.file_url` to destination driver after verified upload, skip-path backfills file_url on idempotent re-runs. Cloud thumbnails: `UploadService::generate_thumbnails` now pushes per-size variants to cloud at sibling paths (`2026/05/foo-150x150.jpg`) when active driver is non-local + new filter `mvs_cloud_thumbnail_url` lets BunnyCDN Optimizer / Cloudflare Images customers delegate resizing to the CDN. Direct CDN URLs: new opt-in setting `mvs_cloud_direct_public_urls` (default off) — when on, `SignedUrlService::maybe_direct_cloud_url` + `maybe_direct_cloud_thumbnail_url` short-circuit the gated `/serve` proxy and emit the CDN edge URL directly for public media (browsers fetch from CDN, WP out of hot path). Privacy gate: cloud upload paths now filter `WHERE privacy='public'` — non-public media stays on local until cloud-aware /serve + signed-GET URLs land in 1.3.0. Filter `mvs_cloudops_allow_non_public_to_cloud` for advanced setups with private buckets. New action hook `mvs_after_thumbnail_generation` (existing) now relevant for cloud-thumb listeners. **i18n sweep:** 98 strings wrapped — 5 chat templates, 4 frontend JS files, 3 Interactivity API view scripts (50 toast strings via runtime shim because @wordpress/i18n can't be imported in script modules yet — WP 6.6 limitation); two locale-broken matchers in `bp-activity-media.js` rewritten from `textContent.indexOf('Share')` to data-attribute selectors (`[data-wp-on--click*="lightboxToggleFavorite"]`) — favorite + share buttons now work on every locale; new `mvsConfirmI18n` + `mvsCardBuildersI18n` localizations via `wp_localize_script`; `wp-i18n` classic script enqueued so `window.wp.i18n.__` is reliably present at runtime; `languages/wpmediaverse.pot` regenerated 1 → 1,179 entries (was stale — single `Delete` entry despite 5+ delete contexts). Verified end-to-end on local (driver=bunnycdn): 58 media migrated, 57 with cloud thumbs at correct paths, 4 originals + variants cleaned via `cleanup-local`, all explore feed images render via direct CDN URLs, 0 console errors. Privacy regression caught + fixed during smoke: 2 private media originals had been uploaded to public BunnyCDN bucket — sealed via `$driver->delete()` + reverted file_url column; gate added to all three layers (CLI WHERE, CloudOps defense-in-depth, Pro admin button query). |
| 2026-05-07 | 1.2.1 release | **6 customer fixes + 100k-readiness pass + filename refactor.** Phase 1A privacy: BP activity `hide_sitewide` now follows the most-restrictive of media + album privacy (Zoho #39974). New `mvs_media_privacy_changed` hook fires from `MediaRepository::set` UPDATE; album changes fan out to all linked per-media + bundled gallery activities. Phase 1B CSS: `shared-ui-shell.css` → `shared-ui-frame.css` (+ rtl + .min) and `templates/partials/shared-ui-shell.php` → `shared-ui-frame.php`; legacy enqueue handle `mvs-shared-ui-shell` kept as register-only shim until 1.3.0 (customer WAFs flag the "shell" token, Crisp #NZRSBX). Phase 3A `Services\AdminAggregatesService`: single source of truth for total_media / total_views / storage_size / recent_media; daily-TTL persistent cache (wp_cache primary, transient fallback only when no object cache); 4 prior callsites consolidated. Phase 3B FULLTEXT search: Migrator v13 adds `media_search_ft` on `mvs_media_index(title, description)`, REST `/media?s=` swaps to `MATCH/AGAINST` for ≥3-char terms with LIKE fallback. Phase 3C view retention: setting `mvs_view_retention_days` (default 90, 0=unlimited, max 730), daily cron `mvs_purge_old_views` purges in 50k-row batches. Phase 3D request cache + prefetch on `MediaRepository`: per-request static cache turns repeated `get($id, ...)` into static-array lookups; `prefetch(array $ids)` batches index+meta in 2 queries; ActivityContentIntegration's two N+1 hot loops now pre-warm the cache. Phase 3E REST per_page hardening: new `WPMediaVerse\REST\Pagination::resolve_per_page` clamps every list endpoint (14 callsites) since WP REST silently skips schema `'maximum'` when `sanitize_callback` is set; new Coding Rule #5 grep prevents drift. Phase 3F storage discipline: new Coding Rule #16 (cache backend by cardinality) + bin/coding-rules-check Rules 3-4; first run flagged `MessagingService::set_typing` per-(conv,user) transients → migrated to `wp_cache_set` with dedicated group. Phase 4 filename strategy: new setting `mvs_filename_strategy` enum (`hashed` for fresh installs, `original_sanitized` for upgrades); new `Services\FilenameStrategy` + `mvs_media_meta.original_filename` for display; SignedUrlService Content-Disposition serves the original name even for hashed on-disk files; **existing media is never renamed**. All phases verified end-to-end (PHP unit + browser); 5 coding rules pass on a clean tree. |
| 2026-05-03 | (1.2.0 RC walkthrough — UX completeness pass) | **13 user-visible features wrapped + verified before 1.2.0 ship.** User directive: "fixing existing UX issue in next version is bad — UX audit and fixes must be part of 1.2.0". Each item below is browser-verified per Verify-Per-Item rule (desktop + 390px mobile). (1) **Member Photos block + 4 missing shortcodes** — `mvs/member-photos` auto-resolves user (explicit → BP displayed → post author → current); `[mvs_explore_feed]`, `[mvs_lock_overlay]`, `[mvs_member_photos]`, `[mvs_pdf_viewer]` registered. (2) **Sorting** — `mvs/media-grid` + `[mvs_gallery]` gain `popular`/`views`/`reactions`/`random` sort options + asc/desc + `userId` filter; LEFT JOIN `mvs_media_stats` for popularity sorts. (3) **Lightbox Download** — toolbar Download button + new `POST /mvs/v1/media/{id}/download` REST endpoint, rate-limited 30/min, increments `mvs_media_stats.downloads`. (4) **Lightbox Fullscreen** — toolbar button + `F` shortcut + Fullscreen API. (5) **Lightbox Share** — `POST /mvs/v1/media/{id}/share` increments `mvs_media_stats.shares`; `window.prompt()` fallback removed (was UX bug — third popup) — fallback chain is now `navigator.share` → `navigator.clipboard` → toast error. (6) **Lightbox 6-reaction a11y** — `aria-label` (sentence-form) + `aria-pressed` on each reaction; emoji `aria-hidden`; `role="group" aria-label="Reactions"` wrapper. Lightbox toolbar buttons all gain `aria-label` + `:focus-visible` outlines (`51d95ba`, `c250f75`). (7) **Upload modal polish** — preview tiles show filename + per-tile remove (×); audio file → audio fallback icon; tags input gains 8 popular-tag pills (click appends, no duplicates). (8) **Per-media Edit modal** — settings cog on dashboard cards opens prefilled modal (title / description / privacy / allow_download); `PUT /mvs/v1/media/{id}` accepts new `allow_download` boolean; `prepare_item_for_response` emits `allow_download` (defaults true when meta absent or != '0'). (9) **Search autocomplete on Explore** — WeakMap-debounced (250ms), top-8 title matches via `/media?s=`, ARIA combobox/listbox semantics, ArrowDown/Up/Enter/ESC keyboard nav. (10) **Open Graph + Twitter Card meta** on `/media/{slug}/` — `wp_head` priority 5 emits `og:*` + `twitter:card=summary_large_image`. (11) **Chat panel visibility setting** — new 4-value enum (everywhere/mvs_pages/bp_pages/disabled) under Direct Messages section + `mvs_should_render_chat_panel` filter; new helpers `is_mvs_page()` + `is_bp_page()`. (12) **Global Allow Downloads toggle** under Media Display section — gates the lightbox button + `record_download` REST endpoint. (13) **Bulk Actions in admin All Media** — multi-select toolbar with context-aware actions (Trash filter → Restore + Delete-permanently; else Move-to-Trash); `wp_nonce_field('mvs_bulk_media')`; success notice with count + action; capability + nonce gates; extracted `permanently_delete_media()` helper. **BP notification dedup** — restored `NotificationIntegration` (mirrors `mvs_notification_created` to BP) but added `function_exists('buddypress')` guard around the dashboard `.mvs-notification-bell` markup so BP-active sites only render notifications in the BP nav bell, not both. **A11y pass** — `qa/A11Y-AUDIT-2026-05-03.md` covers 3 audit passes; deferred to 1.2.1: color-contrast theme audit, aria-live toast container, prefers-reduced-motion CSS. |
