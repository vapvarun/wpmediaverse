...
| `Admin\Settings\` | Settings page (7 focused classes) | `SettingsPage`, `SettingsRegistrar`, `GeneralSettingsRegistrar`, `AiSettingsRegistrar`, `FieldRenderer`, `PermissionsManager`, `Sanitizers` |
...
| `includes/Admin/Settings/SettingsRegistrar.php` | Consolidates the remaining settings groups (storage, display, moderation, webhooks, messaging, pages). AI was extracted to `AiSettingsRegistrar` in 1.4.0; the General group to `GeneralSettingsRegistrar` in 2.6.0 (1,189 → 990). | **OPEN — 990 lines** (2026-09-24). Still over the 500-line rule: the next edit extracts another group (display or moderation are the largest), it does not add one. |
