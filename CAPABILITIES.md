...
| Turn private messaging off | **YES** | Settings > Messages > Messages (`mvs_messaging_enabled`, filter of the same name, 2.6.0). `Core/Plugin::init_messaging()` does not boot the engine when it is off: no messaging routes, chat panel, /messages/ page, Message buttons or Pro group chat, and no `messaging` container service, which BuddyNext reads to hide its own Messages. Conversations are kept; GDPR export/erase stay registered. "Who can send messages: Nobody" remains the softer stop (no new messages, old ones readable) |
...
| Trash a document folder with everything in it | **YES (Pro)** | `FolderService::trash()` trashes the subtree's documents through the normal document trash (batched, Action Scheduler with WP-Cron fallback) and marks them, so `restore()` brings back only what the folder took. Trashed names are free (`trash_token` in the unique key, Migrator v21) and a clash restores as "Name (restored)". Auto-purge after 30 days (`mvs_folder_trash_retention_days`), `DELETE /folders/{id}?force=true`, and `can_manage` / `X-MVS-Can-Create-Folder` so clients render from the rule. On a space drive only owners, moderators and admins manage any folder; members manage their own |
